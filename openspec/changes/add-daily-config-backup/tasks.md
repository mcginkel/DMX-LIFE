...
- [ ] 1.3 Wrap the whole snapshot step in try/except: log any failure via
      `current_app.logger`, and let the real write proceed regardless.
- [ ] 1.4 Ensure `backups/` is created if it doesn't exist yet.
- [ ] 1.5 No deletion logic anywhere in this feature - every day's snapshot
...
- [ ] 3.4 Pre-existing manually-named backup files in `backups/` are
